...
#define CAESAR_SHIFT(c, offset)	((((c + 3) - offset) % 26) + offset)
...
int main() {
	...
	char s[] = "Hello, World! abcdefghijklmnopqrstuvwxyz ABCDEFGHIJKLMNOPQRSTUVWXYZ";

	printf("%s\n", s);

	/* How I would solve this using the for loop ( of course no statements (= )
	...
	*/
	...
	/* How to solve this using the cool as macros from my std lib! */

	// done.
	MAP(s, encode(el));

	// print with spaces, cause why not
	FOR_EACH(s, printf("%c ", el));
	NL;
